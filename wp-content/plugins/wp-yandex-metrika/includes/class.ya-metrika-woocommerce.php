...
class YaMetrikaWoocommerce
{
    ...
    private function __construct()
    {
        ...
        YaMetrika::getInstance()->registerTest(function () {
			...
			$brandType = $options['brand_type'];
			$brandSlug = $options['brand_slug'];
	...
	public function registerInlineScripts(){
		...
		wp_add_inline_script(YAM_SLUG.'_woocommerce', "jQuery(document.body).on('wpym_ec_ready', function(){if (typeof wpym !== 'undefined' && wpym.ec) {wpym.ajaxurl = '".esc_js($ajaxurl)."'; wpym.ec.addData(".wp_json_encode($this->jsData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES).");}})");
	}

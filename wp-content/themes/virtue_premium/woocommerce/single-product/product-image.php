...
if ( ! empty( $image_title ) ) {
	...
	$light_title = get_post_field( 'post_title', $post_thumbnail_id );
}
if ( version_compare( WC_VERSION, '3.0', '>' ) ) {
	if ( '1' == virtue_premium_get_option( 'product_gallery_slider' ) ) {
...
if(isset($virtue_premium['product_simg_resize']) && $virtue_premium['product_simg_resize'] == 0) {
	...
	$presizeimage = 1;
		...
		if(isset($virtue_premium['singleproduct_layout']) && $virtue_premium['singleproduct_layout'] == 'largeimg') { 
				if(virtue_display_sidebar()) {$productimgwidth = 407; } else { $productimgwidth = 748;}
		} else {
			if(virtue_display_sidebar()) { $productimgwidth = 334; } else { $productimgwidth = 456; }
		}

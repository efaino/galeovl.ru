...
if ( ! class_exists( 'Virtue_Get_Image' ) ) {
	class Virtue_Get_Image {
        ...
        public function process($id = null, $width = null, $height = null) {
		    ...
		    if( class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'photon' ) ) {
		    	...
		    	return array (
		            ...
		            4 => self::virtue_get_just_srcset_output($id, $image_url, $width, $height),
					5 => self::virtue_get_sizes_output( $width ),
					6 => $image_url,
					7 => $id
		        );
		...
		public static function virtue_get_srcset_output($id = null, $url = null, $width = null, $height = null) {
		    ...
		    if( ! empty( $img_srcset ) ) {
		      	$output = 'srcset="'.esc_attr($img_srcset).'" sizes="'.esc_attr( self::virtue_get_sizes_output( $width ) ).'"';
		    } else {
		      	$output = '';
		    }
		    return $output;
		}
		public static function virtue_get_just_srcset_output($id = null, $url = null, $width = null, $height = null) {
		    $img_srcset = self::virtue_get_image_srcset($id, $url, $width, $height);
		    if(!empty($img_srcset) ) {
		      	$output = esc_attr($img_srcset);
		    } else {
		      	$output = '';
		    }
		    return $output;
		}
		public static function virtue_get_sizes_output( $width = null ) {
			return '(max-width: '.esc_attr($width).'px) 100vw, '.esc_attr($width).'px';
		}
		...
		public static function virtue_get_full_image($id) {
				...
				$image = array (
					...
					4 => self::virtue_get_just_srcset_output($id, $src[0], $src[1], $src[2]),
					5 => self::virtue_get_sizes_output( $src[1] ),
					6 => $src[0],
					7 => $id
				);
				return $image;

...
add_filter('template_include', 'virtue_cartflows_page_template_support', 20);
function virtue_cartflows_page_template_support( $template ) {
	if( is_singular() ) {
		$t_slug = get_page_template_slug();
		if ( $t_slug == 'cartflows-default' || $t_slug == 'cartflows-canvas' ) {
			remove_filter( 'template_include', array( 'Kadence_Wrapping', 'wrap' ), 101 );
		}
	}
	return $template;
...
/**
 * Function to add support for picu plugin.
 */
function virtue_picu_plugin_override( $template ) {
	$cpt = get_post_type();
	if ( 'picu_collection' === $cpt ) {
		remove_filter( 'template_include', array( 'Kadence_Wrapping', 'wrap' ), 101 );
	}
	return $template;
}
add_filter( 'template_include', 'virtue_picu_plugin_override', 20 );
  ...
  global $virtue_premium, $woocommerce;
    ...
    <?php if (class_exists('woocommerce'))  {
        ...
        <a class="menu-cart-btn" title="<?php echo __('Your Cart', 'virtue');?>" href="<?php echo esc_url(wc_get_cart_url()); ?>">
          <div class="kt-cart-container"><i class="icon-cart"></i><span class="kt-cart-total"><?php echo is_object( WC()->cart ) ? WC()->cart->get_cart_contents_count() : ''; ?></span></div>
        </a>
 ...
 *
 * @param string $limit the number limit.
 * @return string containing the excerpt.
 */
function virtue_excerpt( $limit = null ) {
	if ( empty( $limit ) ) {
		$limit = virtue_premium_get_option( 'home_post_word_count' );
	}
...
function next_post_link_plus($args = '') {
	// if ( isset( $args['in_same_tax'] ) && true === $args['in_same_tax'] ) {
...
//    Set anchor title attribute to long post title or custom tooltip text. Supports variable replacement in custom tooltip.
    if ( $r['tooltip'] ) {
      ...
      $tooltip = ' title="' . esc_attr( strip_tags( $tooltip ) ) . '"';
    } else
      $tooltip = '';

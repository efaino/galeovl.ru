...
function virtue_woocommerce_single_product_hooks() {
	...
	function virtue_woo_rename_tabs( $tabs ) {
		global $virtue_premium;
		if ( ! empty( $virtue_premium['description_tab_text'] ) && ! empty( $tabs['description']['title'] ) ) {
			$tabs['description']['title'] = $virtue_premium['description_tab_text'];}
		if ( ! empty( $virtue_premium['additional_information_tab_text'] ) && ! empty( $tabs['additional_information']['title'] ) ) {
			$tabs['additional_information']['title'] = $virtue_premium['additional_information_tab_text'];}
		if ( ! empty( $virtue_premium['reviews_tab_text'] ) && ! empty( $tabs['reviews']['title'] ) ) {
			$tabs['reviews']['title'] = $virtue_premium['reviews_tab_text'];}
	...
	function virtue_description_tab_heading( $title ) {
		global $virtue_premium;
		if ( ! empty( $virtue_premium['description_header_text'] ) ) {
			$title = $virtue_premium['description_header_text'];}
		return $title;
	}
	...
	function virtue_additional_information_tab_heading( $title ) {
		global $virtue_premium;
		if ( ! empty( $virtue_premium['additional_information_header_text'] ) ) {
			$title = $virtue_premium['additional_information_header_text'];}
		return $title;
	}
	...
	function virtue_woo_reorder_tabs( $tabs ) {
		global $virtue_premium;
		if ( isset( $virtue_premium['ptab_description'] ) ) {
			$dpriority = $virtue_premium['ptab_description'];
		} else {
			$dpriority = 10;}
		if ( isset( $virtue_premium['ptab_additional'] ) ) {
			$apriority = $virtue_premium['ptab_additional'];
		} else {
			$apriority = 20;}
		if ( isset( $virtue_premium['ptab_reviews'] ) ) {
			$rpriority = $virtue_premium['ptab_reviews'];
		} else {
			$rpriority = 30;}
		if ( isset( $virtue_premium['ptab_video'] ) ) {
			$vpriority = $virtue_premium['ptab_video'];
		} else {
			$vpriority = 40;}
		...
		if ( ! empty( $tabs['video_tab'] ) ) {
			$tabs['video_tab']['priority'] = $vpriority;      // Video second
		}
	...
	function virtue_related_products() {
		if ( '0' == virtue_premium_get_option( 'related_products' ) ) {
	 ...
	 */
	function virtue_woo_related_products_limit( $args ) {
		$args['posts_per_page'] = 8;
		return $args;
	}
	add_filter( 'woocommerce_output_related_products_args', 'virtue_woo_related_products_limit' );
	 ...
	 * @param array $args the query args.
	 */
	function virtue_woo_related_products_columns( $columns ) {
		$columns = virtue_premium_get_option( 'related_item_column', '4' );
		return $columns;
	}
	add_filter( 'woocommerce_related_products_columns', 'virtue_woo_related_products_columns' );

	add_action( 'woocommerce_before_main_content', 'virtue_woo_product_breadcrumbs', 20 );
	function virtue_woo_product_breadcrumbs() {
		if ( is_product() ) {
	...
	function virtue_woo_product_gallery_sizes( $size ) {
		if ( '1' === virtue_premium_get_option( 'product_simg_resize' ) ) {
			$img_ratio = virtue_premium_get_option( 'shop_img_ratio' );
			if ( 'largeimg' === virtue_premium_get_option( 'product_simg_resize' ) ) {
				$productimgwidth = 748;
			} else {
				$productimgwidth = 456;
			}

			if ( 'portrait' === $img_ratio ) {
				$tempproductimgheight = $productimgwidth * 1.35;
				$productimgheight     = floor( $tempproductimgheight );
			} elseif ( 'landscape' === $img_ratio ) {
				$tempproductimgheight = $productimgwidth / 1.35;
				$productimgheight     = floor( $tempproductimgheight );
			} elseif ( 'widelandscape' === $img_ratio ) {
	...
	add_filter( 'woocommerce_get_image_size_single', 'virtue_woo_product_gallery_sizes' );
}
...
function kad_product_custom_tab_content_01() {
	...
	$tab_content_01 = get_post_meta( $post->ID, '_kad_tab_content_01', true );
	if ( isset( $wp_embed ) ) {
		$tab_content_01 = $wp_embed->autoembed( $tab_content_01 );
	}
	echo do_shortcode( '<div class="product_custom_content_case">' . wpautop( apply_filters( 'kadence_custom_woo_tab_01_content', $tab_content_01 ) ) . '</div>' );
}
...
function kad_product_custom_tab_content_02() {
	...
	if ( isset( $wp_embed ) ) {
		$tab_content_02 = $wp_embed->autoembed( $tab_content_02 );
	}
...
function kad_product_custom_tab_content_03() {
	...
	if ( isset( $wp_embed ) ) {
		$tab_content_03 = $wp_embed->autoembed( $tab_content_03 );
	}

...
jQuery(window).load( function () {
	function kadence_build_map( $jcontainer ) {
		var isDraggable = jQuery( document).width() > 480 ? true : false;
		var scrollwheel = $jcontainer.data('map-scrollwheel');
		var maptype = $jcontainer.data('maptype');
		var mapzoom = $jcontainer.data('mapzoom');
		var address1 = $jcontainer.data('address1');
		var address2 = $jcontainer.data('address2');
		var address3 = $jcontainer.data('address3');
		var address4 = $jcontainer.data('address4');
		var position1 = $jcontainer.data('position1');
		var position2 = $jcontainer.data('position2');
		var position3 = $jcontainer.data('position3');
		var position4 = $jcontainer.data('position4');
		var title1 = $jcontainer.data('title1');
		var title2 = $jcontainer.data('title2');
		var title3 = $jcontainer.data('title3');
		var title4 = $jcontainer.data('title4');
		var mapcenter = $jcontainer.data('mapcenter');
		var geocode = $jcontainer.data('geocode');
		if( maptype == 'ROADMAP' ) {
		...
		if ( position1 ) {
			values = [{
				...
				data   : ( title1 ? '<div class="mapinfo"><h5>' + title1 + '</h5>' + address1 + '</div>' : '<div class="mapinfo">' + address1 + '</div>' ),
			}];
		} else {
			values = [{
				address: address1,
		...
		if ( address2 || position2 ) {
			if ( position2 ) {
				values.push({
					position: position2.split(','),
					data   : ( title2 ? '<div class="mapinfo"><h5>' + title2 + '</h5>' + address2 + '</div>' : '<div class="mapinfo">' + address2 + '</div>' ),
				});
			} else {
				values.push({
					address: address2,
		...
		if ( address3 || position3 ) {
			if ( position3 ) {
				values.push({
					position: position3.split(','),
					data   : ( title3 ? '<div class="mapinfo"><h5>' + title3 + '</h5>' + address3 + '</div>' : '<div class="mapinfo">' + address3 + '</div>' ),
				});
			} else {
				values.push({
					address: address3,
		...
		if ( address4 || position4 ) {
			if ( position4 ) {
				values.push({
					position: position4.split(','),
					data   : ( title4 ? '<div class="mapinfo"><h5>' + title4 + '</h5>' + address4 + '</div>' : '<div class="mapinfo">' + address4 + '</div>' ),
				});
			} else {
				values.push({
					address: address4,
		...
		$jcontainer.gmap3(args).marker(values).infowindow({
	        content: '',
	    })
	...
	jQuery('.kt-gmap-js-init').each(function() {
		kadence_build_map( jQuery(this) );
	});
});